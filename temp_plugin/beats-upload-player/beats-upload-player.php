...
<!-- wp:beats/visualizer /-->
<!-- wp:html -->
<div id="beats-wrapper" class="beats-wrapper" style="padding-right:0">
<!-- wp:group {"align":"full","style":{"spacing":{"blockGap":"16px"}},"layout":{"type":"flex","orientation":"vertical"}} -->
<div class="wp-block-group alignfull" style="gap:16px;width:100%;padding-left:20px;">
...
/**
 * Map each dynamic block slug to the shortcode tag that should render it.
 */
function beats_get_block_shortcode_map() {
  $map = array(
  ...
  return apply_filters( 'beats_block_shortcode_map', $map );
}

/**
 * Render a shortcode string on behalf of a block render callback.
 *
 * @param string $shortcode      Shortcode tag.
 * @param array  $attributes     Optional attributes passed from the block.
 * @param bool   $show_notice    Whether to show a warning when the shortcode is missing.
 *
 * @return string Rendered shortcode output or empty string on failure.
 */
function beats_render_shortcode_block( $shortcode, $attributes = array(), $show_notice = true ) {
  if ( empty( $shortcode ) ) {
    return '';
  }

  if ( ! shortcode_exists( $shortcode ) ) {
    if ( $show_notice && current_user_can( 'edit_posts' ) ) {
      return sprintf(
        '<div class="notice notice-warning">%s</div>',
        esc_html(
          sprintf(
            /* translators: %s: shortcode tag */
            __( 'The %s shortcode is not available in this environment.', 'beats-upload-player' ),
            $shortcode
          )
        )
      );
  ...
  foreach ( (array) $attributes as $key => $value ) {
    ...
    $pairs[] = sanitize_key( $key ) . '="' . esc_attr( $value ) . '"';
  }
  ...
  $shortcode_string = sprintf( '[%s%s]', $shortcode, $attr_string );

  return shortcode_unautop( do_shortcode( $shortcode_string ) );
}
...
function beats_register_core_blocks() {
  ...
  foreach ( beats_get_block_shortcode_map() as $slug => $shortcode ) {
    register_block_type_from_metadata(
      __DIR__ . '/blocks/' . $slug,
      array(
        'render_callback' => function( $attributes ) use ( $shortcode ) {
          return beats_render_shortcode_block( $shortcode, $attributes );
        },
      )
    );
  }
...
function beats_enqueue_admin_assets($hook) {
  $valid_hooks = [
    ...
    'beats-manager_page_beats-manager-global-player',
    'beats-manager_page_beats-manager-search-settings',
  ];
