...
(function () {
    ...
    return $(`
      <div class="beat-card-admin" data-file="${beat.file}">
        ...
        <div class="fields">
          ...
          <label>Stripe Buy Link</label>
          <input class="fld-buy" type="url" value="${escapeHtml(beat.buy_url || '')}" placeholder="https://checkout.stripe.com/...">
        </div>
  ...
  app.on('click', '.btn-save', function () {
    ...
    const buyLink = card.find('.fld-buy').val();

    card.addClass('saving');
    api('beats_update', { file, name, producer, category: cat, price, buy_url: buyLink })
      .done(res => {
        if (!res || !res.success) {
