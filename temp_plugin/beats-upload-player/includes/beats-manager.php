...
function beats_admin_handle_search_bar_settings() {
  if ($_SERVER['REQUEST_METHOD'] !== 'POST' || empty($_POST['beats_cltd_search_settings'])) {
    return '';
  }

  if (!current_user_can('manage_options')) {
    return '<div class="notice notice-error"><p>' . esc_html__('You do not have permission to update the search bar.', 'beats-upload-player') . '</p></div>';
  }
  ...
  $enabled = isset($_POST['beats_enable_search_bar']) ? '1' : '0';
  $sticky  = isset($_POST['beats_disable_sticky_search_bar']) ? '1' : '0';

  update_option(beats_cltd_search_enable_option_key(), $enabled);
  update_option(beats_cltd_search_sticky_option_key(), $sticky);

  return '<div class="notice notice-success is-dismissible"><p>' . esc_html__('Search bar settings saved.', 'beats-upload-player') . '</p></div>';
}
...
function beats_admin_page_render($visible_sections = null, $scroll_target = '') {
  ...
  $search_notice = beats_admin_handle_search_bar_settings();
  ...
  if ($show_search) {
    echo beats_admin_render_search_settings_section($search_enabled, $sticky_disabled);
  }
...
function beats_admin_render_search_settings_section($search_enabled, $sticky_disabled) {
  ob_start();
  ?>
  <div id="beats-admin-search" class="beats-admin-search card">
    <h2><?php esc_html_e('Search Bar Settings', 'beats-upload-player'); ?></h2>
    <p><?php esc_html_e('Control whether the frontend library shows the search bar and whether it sticks to the top while scrolling.', 'beats-upload-player'); ?></p>
    <form method="post">
      <?php wp_nonce_field('beats-search-settings', 'beats_cltd_search_settings_nonce'); ?>
      <input type="hidden" name="beats_cltd_search_settings" value="1">

      <label for="beats_enable_search_bar" style="display:flex;align-items:center;gap:8px;">
        <input type="checkbox" id="beats_enable_search_bar" name="beats_enable_search_bar" value="1" <?php checked($search_enabled, true); ?>>
        <?php esc_html_e('Enable Search Bar', 'beats-upload-player'); ?>
      </label>

      <label for="beats_disable_sticky_search_bar" style="display:flex;align-items:center;gap:8px;margin-top:12px;">
        <input type="checkbox" id="beats_disable_sticky_search_bar" name="beats_disable_sticky_search_bar" value="1" <?php checked($sticky_disabled, true); ?>>
        <?php esc_html_e('Disable Sticky Search Bar', 'beats-upload-player'); ?>
      </label>

      <p class="description" style="margin-top:12px;"><?php esc_html_e('When sticky mode is enabled, the search bar stays pinned near the top of the screen while scrolling.', 'beats-upload-player'); ?></p>

      <p><button type="submit" class="button button-primary"><?php esc_html_e('Save Search Settings', 'beats-upload-player'); ?></button></p>
    </form>
  </div>
  <?php
  return ob_get_clean();
