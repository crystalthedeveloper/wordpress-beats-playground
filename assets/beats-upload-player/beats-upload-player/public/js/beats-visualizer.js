...
function BeatsVisualizerInit() {
  ...
  const container = document.getElementById("beats-visualizer-container");
  const config = window.BeatsVisualizerConfig || {};

  function parseContainerColors() {
    if (!container) return [];
    const serialized = container.getAttribute("data-band-colors");
  ...
  let bandColors = parseContainerColors();
  if (!bandColors.length && Array.isArray(config.colors)) {
    bandColors = config.colors;
  }
