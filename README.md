...
- **Template override:** the blueprint overwrites the Twenty Twenty-Five `blog-home` template (the one Playground actually loads by default) with a lightweight block that pulls in the stock header/footer and drops four shortcode blocks (`[beats_category_search]`, `[beats_visualizer]`, `[beats_display_home]`, `[beats_global_player]`) in between, so the Beats UI renders instantly on the homepage and is editable inside the Site Editor.  
