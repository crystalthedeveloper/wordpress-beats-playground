...
- **Landing page:** `createPost` builds the Beats Playground page (with all of the shortcodes shown above), `setHomepage` promotes it to the front page, and a small `runPHP` step flushes rewrite rules + redirects to `/` so the page loads immediately without cached 404s.

---

## ✏️ Customize the Demo

- **Modify the page content:** edit the HTML/shortcodes inside the `createPost` block to change the layout or add/remove shortcodes.  
