...
- **Template override:** the blueprint overwrites the Twenty Twenty-Five `home`, `front-page`, and `index` templates with a lightweight block that pulls in the stock header/footer and drops the Beats section in between, ensuring `#beats-wrapper` is present as soon as the page loads.  
